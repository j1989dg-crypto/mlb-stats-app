...
async def get_game_analysis(game_pk: int, refresh: bool = False):
    ...
    # Pre-calculate baseline metrics for the AI prompts (allows parallel execution)
    try:
        away_k9 = float(away_pitcher_profile.get("k9", 8.0) or 8.0)
    except:
        away_k9 = 8.0
    try:
        home_k9 = float(home_pitcher_profile.get("k9", 8.0) or 8.0)
    except:
        home_k9 = 8.0
    away_k_proj = round(home_k9 * 0.8 + 2.0, 1)
    home_k_proj = round(away_k9 * 0.8 + 2.0, 1)
    if away_k_proj > home_k_proj + 0.5:
        strikeout_lean = away_team.get("name", "Away")
    elif home_k_proj > away_k_proj + 0.5:
        strikeout_lean = home_team.get("name", "Home")
    else:
        strikeout_lean = "Even"
    ...
    game_defining = f"Tactical battle: {star_away} vs {home_pitcher_profile.get('name', 'Home SP')}, and {star_home} vs {away_pitcher_profile.get('name', 'Away SP')}."

    shared_payload["pitch_matchup_analysis"] = {
    ...
    # Execute all three AI analysis calls in parallel
    pitch_matchup_ai, ai_analysis, ai_betting = await asyncio.gather(
        analyze_pitch_matchups(game_pk, shared_payload),
